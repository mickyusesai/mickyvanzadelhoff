...
**Status (2026-09-19, session 9):** Phases 0–5 (pre-launch part), 6 (launch: real 301s merged and deployed, old domain redirecting via Cloudflare, verified), 7 (illustrated headers) and 8 (repositioning + spot illustrations) done. Phase 5 post-launch batch 1 (five affiliate articles) refreshed and fact-checked (55 of 58 notes resolved by Claude, 3 partner-link tasks left) and batch 2 (ten ondernemen articles) refreshed on 2026-09-16; briefing 2 applied (`/automatisering/` with spot illustrations, `/intake/` form, D26/D27); the crypto guide deepened with live widgets on 2026-09-17 (D29) and its Bitcoin chart fixed the same day (the TradingView script overwrites its container height, so the fixed-height frame now wraps the container); the intake mail moved from SMTP to Postmark's HTTPS API because Railway blocks SMTP on the Hobby plan (D30); everything is merged (PR #16) and Micky confirmed on 2026-09-19 that the intake form delivers. Batch 3 (28 articles: the remaining ondernemen and all online-geld-verdienen) refreshed on 2026-09-19 to the crypto-guide level (D31): every fact re-verified or removed, a calculator and/or timeline per article, comparison tables with check dates, 111 generated scene illustrations replacing stock images and 2019 screenshots. Favicon chosen (lightning bolt). Possibol links replaced by Business Mine. Open on Micky's side: Phase 6 steps 5–8 (www record, Search Console change of address, cancel Cloud86 hosting, post-launch monitoring), the Technogym workshop original for /over/ and the homepage 'Ik bouw' block (the 21 MB PNG in the Drive folder 'Claude' is over the 10 MB Drive-tool limit; share it as 'anyone with the link' or add a JPG), rotate the fal.ai key. The homepage hero and the blog author box now use `micky-van-zadelhoff-portret.webp`, a square crop of `IMG_1680.JPG` from that folder (2026-09-15).
**Next:** Micky answers the batch 3 decision list (see D31 and the board); the Technogym photo waits (Micky, 2026-09-19). Remaining refresh work: digitalenomaden (20), review (5 incl. Money Mastery, Binance, Siteground, Cloud86, Enormail), web3 rest (beste-crypto-exchange, gratis cursussen, nft-uitleg, …), tips.
...
| D31 | Batch 3 at crypto-guide level | Micky, 2026-09-19: "op dezelfde manier te werk gaan als bij geld verdienen met crypto". 28 articles refreshed by seven parallel agents following `scratchpad/batch3-recipe.md` (kept in the session, summarised here): verify every fact against the vendor or a primary source, remove what cannot be verified, first-person Dutch, intro with what the reader learns, at least one living element (`.tool` calculator with inline script, `.timeline`, TradingView `.embed`, dated comparison tables), generated scene illustrations for decorative/outdated images while Micky's own screenshots stay with a date in the caption, every `/go/` link kept (dead partner: TODO comment). New CSS: `.tool*` classes in global.css. Open for Micky: see the decision list in the board (eToro still used?, Van Blog Naar Business still sold?, cancelled 2023 Italy bootcamp mention, be-innovative → curatua.nl, Fiverr gig deep links unverifiable, /go/nft-secrets dead, Shopify NL price, affiliate income framed as past 2023 peak). Also: Possibol → Business Mine partner link (D: both /go/ bol.com links), favicon = generated lightning bolt. |
